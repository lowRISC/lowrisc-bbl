...
int testrange(void volatile *aligned, size_t bufsize, ul loops, int narrow) {
    ...
    printm("test range is 0x%x to 0x%x\n", aligned, aligned+bufsize-1);
...
void dramtest(void)
{
    ...
    printm("memtester version " __version__ " (%d-bit)\n", UL_LEN);
    printm("Copyright (C) 2001-2012 Charles Cazabon.\n");
    printm("Licensed under the GNU General Public License version 2 (only).\n");
    printm("\n");
    testrange((void volatile *) 0x80F00000, 0x7000000, 1, 1);
}

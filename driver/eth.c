...
static int dummy_eth_read(int pingpong, size_t addr)
{
  int ret = 0;
  size_t paddr = (pingpong&1 ? XEL_BUFFER_OFFSET : 0) | addr;
  printf("dummy_eth_read(%x);\n", paddr);
  return ret;
}

static void dummy_eth_write(int pingpong, size_t addr, int data, int strb)
{
  size_t paddr = (pingpong&1 ? XEL_BUFFER_OFFSET : 0) | addr;
  printf("dummy_eth_write(%x,%x);\n", paddr, data);
}

static eth_read_t eth_read = dummy_eth_read;
static eth_write_t eth_write = dummy_eth_write;
...
static void copyin_pkt(int pp, int length)
{
  ...
  for (i = 0; i < (((length-1)|3)+1)/4; i++)
	  {
            ((uint32_t *)uip_buf)[i] = eth_read(pp, XEL_RXBUFF_OFFSET+(i<<2));
	  }
  uip_len = length;
}
...
static void lite_copyout(int pp, int length)
{
  ...
  for (i = 0; i < (((length-1)|3)+1)/4; i++)
	  {
	    eth_write(pp, XEL_TXBUFF_OFFSET+(i<<2), ((uint32_t *)uip_buf)[i], 0xf);
 	  }
}
...
{
  int rslt;
  eth_write(pp, XEL_TPLR_OFFSET,length,0xf);
  eth_write(pp, XEL_TSR_OFFSET,XEL_TSR_XMIT_BUSY_MASK,0xf);
 do
   {
     rslt = eth_read(pp, XEL_TSR_OFFSET);
#ifdef VERBOSE
     printf("TX Status = %x\n", rslt);
#endif
   }
 ...
 eth_write(pp, XEL_TSR_OFFSET, rslt, 0xf);
}

static void eth_dummy(int pp)
{
  eth_write(pp, 0x00000000,0xffffffff,0xf);
  eth_write(pp, 0x00000004,0xffffffff,0xf);
  eth_write(pp, 0x00000008,0xffffffff,0xf);
  eth_write(pp, 0x0000000c,0x11111111,0xf);
  eth_write(pp, 0x00000010,0x22222222,0xf);
  eth_write(pp, 0x00000014,0x33333333,0xf);
  eth_read(pp, 0x0000001c);
  eth_write(pp, 0x00000020,0x55555555,0xf);
  lite_xmit(pp, 0x14);
}

static int eth_poll(int pp)
{
  int i, rslt, lenmask, proto_type, length, ret = 0;
  lenmask = eth_read(pp, XEL_RSR_OFFSET);
  //printf("RX Status = %x\n", lenmask);
  /* Check if there is Rx Data available */
  if (lenmask & XEL_RSR_RECV_DONE_MASK)
    {
      int header = eth_read(pp, XEL_HEADER_OFFSET + XEL_RXBUFF_OFFSET);
      ret = 1;
#ifdef VERBOSE
      printf("header = %x\n", header);
#endif
      ...
      printf("length = %d\n", length);
#endif
      eth_write(pp, XEL_RSR_OFFSET, lenmask & ~XEL_RSR_RECV_DONE_MASK, 0xf);
    }
  return ret;
}
...
void eth_init(mini_putchar_t mini_putchar_ptr, eth_read_t eth_read_ptr, eth_write_t eth_write_ptr) {
  uip_ipaddr_t addr;
  mini_putchar = mini_putchar_ptr;
  eth_read = eth_read_ptr;
  eth_write = eth_write_ptr;
  ...
  eth_write(0, XEL_RSR_OFFSET, XEL_RSR_RECV_IE_MASK,0xf);
  eth_write(1, XEL_RSR_OFFSET, XEL_RSR_RECV_IE_MASK,0xf);
  eth_write(0, XEL_TSR_OFFSET, XEL_TSR_XMIT_IE_MASK,0xf);
  eth_write(1, XEL_TSR_OFFSET, XEL_TSR_XMIT_IE_MASK,0xf);
  eth_write(0, XEL_GIER_OFFSET, XEL_GIER_GIE_MASK,0xf);
